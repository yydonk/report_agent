...
### <mark>frame文件夹下有某学校某学院很完善的实验报告模板和生成的实验报告实例，newframe是通用模板，frame2026是电子学基础实验模板，模板可以拿来直接用，效果最好。如有个性化模板要求，尽量在提供的实验模板上改动，防止LLM识别失误</mark>
...
**解析兜底**
- 模板章节解析为空时，自动加载 `frame/default_content_schema.json` 作为章节结构兜底
- 段落格式识别为空时，自动加载 `frame/default_format_schema.json` 作为格式规范兜底

**解析结果可见 / 可导出 / 可导入**
- 解析完成后，可在「查看 / 导出解析结果 JSON」面板预览 content_schema 和 format_schema
- 支持一键下载两个 JSON 文件，下次直接导入跳过重新解析
- 导入区新增「使用内置默认 JSON」按钮，一键加载内置模板结构

**章节管理**
- 草稿生成阶段支持删除/撤销章节，删除的章节不参与生成
- 新增「撤销全部删除」按钮

**跨章节去重**
- 逐章节生成时，将已生成章节的摘要作为上下文传入，引导模型避免重复内容

### 内置模板更新（frame/）

**newframe.docx**
- 重写为 AI 友好模板：使用标准 Heading 样式（解析器可正确识别）、删除空壳章节、补充各章详细写作指令和最低字数要求

**frame2026.docx（新增）**
- 基于 `实验报告模板2026.docx` 重写的 AI 友好版本，适用于电路类实验
- 每章补充结构化写作指令（含验证型/设计型区分、仿真软件说明等）
